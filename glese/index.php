...
if(isset($_GET['action'])){
    ...
    switch($action){
        case 'entente' :
            ...
            break;
        case 'valider' :
            $listeCandidatOffre = getListByOffrev($idOffre);
            break;
        case 'refuser' :
            $listeCandidatOffre = getListByOffrer($idOffre);
            break;
    }
}
...
if (isset($_POST['postuler'])) {
   if(empty($_SESSION)){
        ...
        if ($res=1) {
            header("location:http://localhost/glese/?page=detailOffre&idOffre=1");
        }
   }
}
...
if(isset($_GET['idOffre'],$_GET['etatC'],$_GET['idUser'])){
    extract($_GET);
    updateCandidature($_GET['idOffre'],$_GET['etatC'],$_GET['idUser']);
    $req=pres($_GET['idUser']);
    $to=$req['email'];
    $subject="Utilisation de mail() avec php depuis Windows";
    $salut="vous avez accepter à la poste";
    $header="content-type : text/plain; charset=utf-8\r\n";
    $header.="From:bayebara2000@gmail.com";
    if(mail($to,$subject,$salut,$header)){
        header("location:http://localhost/glese/?page=detailOffre&id=$id");
